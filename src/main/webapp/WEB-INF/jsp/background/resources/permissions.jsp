...
function closeWin(){
	layer.confirm('是否关闭窗口？', function(index) {
...
function sub(){
	if ("${param.roleId}" != "") {
		...
		ly.ajax({
			...
			type : "POST",
			data : $("#from").serialize(),
			url : rootPath + '/background/resources/addRoleRes.shtml',
			dataType : 'json',
		...
		layer.alert("该用户还没有分配角色或用户的角色被删除了，请重新分配角色！！", 3);
	};
}
</script>
<body>
...
<a href="#" class="btn btn-s-md btn-info btn-rounded" onclick="closeWin();">关闭</a>
	...
	$.ajax({
		...
		type : "POST",
		//data : $("#from").serialize(),
		url : rootPath + '/background/resources/findRoleRes.shtml?roleId=${param.roleId}',
		dataType : 'json',
		success : function(json) {
			for(index in json){
				$("input:checkbox[value='"+json[index].id+"']").prop('checked','true');
			}
		}
	});
