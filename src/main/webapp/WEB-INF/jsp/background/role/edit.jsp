...
<script type="text/javascript" src="${ctx}/js/role/edit.js"></script>
<style type="text/css">
.col-sm-3 {
	width: 15%;
	float: left;
...
<body>
	...
	<form id="form" name="form" class="form-horizontal" method="post"
		action="${ctx}/background/role/update.shtml">
					...
					<div class="col-sm-9">
						<div class="btn-group m-r">
							<button data-toggle="dropdown"
								...
								<span class="dropdown-label"><c:if test="${role.enable eq 0}">是</c:if><c:if test="${role.enable eq 1}">否</c:if></span> <span class="caret"></span>
							</button>
							<ul class="dropdown-menu dropdown-select">
								<li class=""><a href="#"><input type="radio"
										name="enable" value="0"<c:if test="${role.enable eq 0}"> checked="checked"</c:if>>是</a></li>
								<li class="active"><a href="#"><input type="radio"
										name="enable" value="1" <c:if test="${role.enable eq 1}"> checked="checked"</c:if>>否</a></li>
							</ul>
						</div>
					</div>
				</div>
