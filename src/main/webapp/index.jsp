...
window.onresize = function()  
   ...
   function tobase(){
	   var bo = $(".navbar-fixed-top-xs+*").css("padding-top");
       if(bo=="50px"){
       	if(fifm==0){
	        	var h = $("#ifm").height()-50;
	        	$("#ifm").height(h);
		...
		<header class="bg-dark dk header navbar navbar-fixed-top-xs">
			<div class="navbar-header aside-md">
				<a class="btn btn-link visible-xs"
					...
					src="${ctx}/notebook/notebook_files/logo.png" class="m-r-sm">飓风商贸</a>
			...
			<ul class="nav navbar-nav navbar-right m-n hidden-xs nav-user">
				...
				<li class="dropdown"><a href="index.html#"
					...
					<ul class="dropdown-menu animated fadeInRight">
						...
						<li class="divider"></li>
						<li><a href="logout.shtml" >Logout</a>
						</li>
					</ul></li>
			</ul>
		</header>
								...
								<nav class="nav-primary hidden-xs">
									<ul class="nav">
										<c:forEach var="key" items="${list}" varStatus="s">
											...
											<li <c:if test="${s.index==0}">class="active"</c:if>><a href="javascript:void(0)"
												...
												<c:forEach var="kc" items="${key.children}">
													<li class="active"><a href="javascript:toiframe('${ctx}${kc.resUrl}')" class="active">
