...
(function() {
	lanyuan = {};
	lanyuan.lyGrid = (function(params) {
		var confs = {
			l_column : [],
			...
			$.each(json, function(d) {
				...
				$.each(column, function(o) {
					if (!column[o].hide || column[o].hide == undefined) {
						...
						var data = _getValueByName(rowdata, clm);
		...
		var fenyeDiv = function(divid, jsonData) {
			...
			bdiv.setAttribute("style", "vertical-align: middle;border: 1px solid #DDDDDD;");
						...
						if (!column[o].hide || column[o].hide == undefined) {
							...
							if (l_tree.tree) {
								if (l_tree.name == column[o].colkey) {
									...
									divspan.className = "l_test";
									divspan.setAttribute("style", "line-height:" + conf.tbodyHeight + ";");
									divspan.innerHTML = _getValueByName(jsonTree[jt], column[o].colkey);
									td_o.appendChild(divspan);
								} else {
									td_o.innerHTML = _getValueByName(jsonTree[jt], column[o].colkey);
								}
								;
							} else {
								td_o.innerHTML = _getValueByName(jsonTree[jt], column[o].colkey);
		...
		var lyGridDown = function(jsonUrl) { // 下移所选行
			...
			var downOne = function(tr) {
				if (tr.rowIndex < divid.children[1].children.mytable.rows.length - 1) {
					swapTr(tr, divid.children[1].children.mytable.rows[tr.rowIndex + 1]);
					getChkBox(tr).checked = true;
				}
			};
